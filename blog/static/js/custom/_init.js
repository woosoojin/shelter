...
function proguards_ready_actions() {
	...
	jQuery('.woocommerce .mode_buttons a,.woocommerce-page .mode_buttons a').click(function(e) {
		...
		jQuery.cookie('proguards_shop_mode', mode, {expires: 365, path: '/'});
...
function proguards_scroll_actions() {
	...
	//alert(jQuery('.top_panel_wrap').height());
...
function proguards_video_dimensions() {
	jQuery('.sc_video_frame').each(function() {
		...
		if (parseInt(frame.attr('data-last-width'))==w_real,10) return;
		if (percent) {
			frame.height(h_real);
		} else {
	...
	jQuery('video.sc_video,video.wp-video-shortcode').each(function() {
		...
		var w_real = Math.round(mejs_cont.length > 0 ? Math.min(percent ? 10000 : w_attr, mejs_cont.parents('div,article').width()) : video.width()),
			h_real = Math.round(percent ? w_real/ratio : w_real/w_attr*h_attr);
		if (parseInt(video.attr('data-last-width'))==w_real,10) return;
	...
	jQuery('video.sc_video_bg').each(function() {
		...
		if (parseInt(video.attr('data-last-width'))==w1,10) return;
	...
	jQuery('iframe').each(function() {
		...
		var w_real = frame.length > 0 ? frame.width() : iframe.width(),
			h_real = Math.round(percent ? w_real/ratio : w_real/w_attr*h_attr);
		if (parseInt(iframe.attr('data-last-width'))==w_real,10) return;
